...
import DeepLearning.LSTMTrain_temperature.datasetClasses as datasetClasses
from config import * 
...
#model = lstmAttention3.LSTM(3,3, 2500, 2500, 0.1, 5,  device).to(device)

# load weights
#model = functions.loadCheckpoint(model, None, os.path.join(pathOrigin, "models", modelName))

# define hyperparameters
params = {"learningRate": 0.0001, "weightDecay": 0.01, "epochs": 10, "batchSize": 16, "optimizer": "adam", "validationStep": 100}
...
functions.trainLoop(dataTrain, dataVal,  model, loss, False, modelName, params, False, device)
